...
const AddPackageModal = ({ show, onHide }) => {
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm, setFieldValue, validateForm }) => {
    setError("");
    setSuccess("");
    ...
    try {
      ...
      // Only upload gallery images if new files were selected
      if (galleryFiles.length > 0) {
        const uploadedUrls = await uploadGalleryImages(setFieldValue, values);
        if (uploadedUrls.length === 0 && galleryFiles.length > 0) {
          setSubmitting(false);
          return;
        }
        // Gallery URLs are already updated in the form by the uploadGalleryImages function
      }

      // Create package with image URLs
      const packageData = {
        ...values,
        img: mainImageUrl,
        images: values.images, // Use the current form values which include any newly uploaded images
      };

      // Use the createPackage API function
                  ...
                  <Tab.Pane eventKey="images">
                    <Row className="mb-4">
                      <Col>
                            ...
                            <Form.Group controlId="mainImageUpload">
                              ...
                              {mainImageFile && (
                                ...
                                >
                                  ...
                                  {uploadingMain ? "Uploading..." : "Upload Now"}
                                </Button>
                              )}
                            </Form.Group>
                          </div>
                        ...
                        <Form.Group className="mb-3">
                          <Form.Label>
                            Main Image URL (or upload above)
                          </Form.Label>
                          <Form.Control
                            type="text"
                            name="img"
                          ...
                          <Form.Group controlId="galleryImagesUpload">
                            ...
                            {galleryFiles.length > 0 && (
                              <Button
                                ...
                                size="sm"
                                className="ms-2"
                                onClick={() => uploadGalleryImages(setFieldValue, values)}
                                disabled={uploadingGallery}
                              >
                                <i className="bi bi-cloud-upload me-1"></i>
                                {uploadingGallery ? "Uploading..." : "Upload Selected Images"}
                              </Button>
                            )}
                          </Form.Group>
                        </div>
